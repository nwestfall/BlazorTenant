...
namespace BlazorTenant
{
    ...
    internal class LegacyMultiTenantTypeRouteConstraint<T> : LegacyMultiTenantRouteConstraint
    {
        public delegate bool TryParseDelegate(string str, out T result);

        private readonly TryParseDelegate _parser;

        public LegacyMultiTenantTypeRouteConstraint(TryParseDelegate parser)
        {
            _parser = parser;
        }
