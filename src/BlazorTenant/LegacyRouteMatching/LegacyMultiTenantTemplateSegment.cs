// https://github.com/dotnet/aspnetcore/blob/main/src/Components/Components/src/Routing/LegacyRouteMatching/LegacyTemplateSegment.cs
...
{
    internal class LegacyMultiTenantTemplateSegment
    {
        public LegacyMultiTenantTemplateSegment(string template, string segment, bool isParameter)
        {
            ...
            if (!isParameter || Value.IndexOf(':') < 0)
            {
                ...
                Constraints = Array.Empty<LegacyMultiTenantRouteConstraint>();
            }
            else
            {
                ...
                Constraints = tokens.Skip(1)
                    .Select(token => LegacyMultiTenantRouteConstraint.Parse(template, segment, token))
                    .ToArray();
            }
        ...
        public LegacyMultiTenantRouteConstraint[] Constraints { get; }
