// https://github.com/dotnet/aspnetcore/blob/main/src/Components/Components/src/Routing/RouteEntry.cs
...
namespace BlazorTenant
{
    ...
    internal class MultiTenantRouteEntry
    {
        public MultiTenantRouteEntry(MultiTenantRouteTemplate template, Type handler, string[] unusedRouteParameterNames)
        {
            Template = template;
        ...
        public MultiTenantRouteTemplate Template { get; }

        public string[] UnusedRouteParameterNames { get; }

        public Type Handler { get; }

        internal bool Match(string tenant)
        {
            if(Template.Segments.Any(s => s.Value.Equals(tenant, StringComparison.OrdinalIgnoreCase)))
                return true;
            
            return false;
        }

        internal void Match(MultiTenantRouteContext context)
        {
            var pathIndex = 0;
            var templateIndex = 0;
        ...
        private void AddDefaultValues(Dictionary<string, object?>? parameters, int templateIndex, MultiTenantTemplateSegment[] segments)
        {
            if(parameters == null)
                return;
        ...
        private bool RemainingSegmentsAreOptional(int index, MultiTenantTemplateSegment[] segments)
        {
